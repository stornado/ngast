...
public class GithubApiTest extends BaseNgastTestCase {
    ...
    @Test(dataProvider = "fetchRepositoryNames",
        description = "搜索仓库地址",
        suiteName = "Github Api Test",
        testName = "Search Repositories",
        timeOut = 10_000)
    @Story("示例搜索")
    @Description("参数化搜索仓库")
    @Step("搜索第一页数据")
    public void testSearchRepositories(String name, String sort) {
        ...
        log.debug(respJsonPath.getString("items[2].owner.login"));

        given()
            .get("/repos/{owner}/{repo}",
                respJsonPath.getString("items[2].owner.login"),
                respJsonPath.getString("items[2].name"))
            .then()
            .statusCode(HttpStatus.SC_OK)
            .time(lessThanOrEqualTo(15L), TimeUnit.SECONDS)
            .contentType(ContentType.JSON)
            ...
            .body("stargazers_count", equalTo(respJsonPath.getInt("items[2].stargazers_count")))
            .body("id", equalTo(respJsonPath.getInt("items[2].id")))
            .body("node_id", equalTo(respJsonPath.getString("items[2].node_id")))
            .body("full_name", equalTo(respJsonPath.getString("items[2].full_name")));
